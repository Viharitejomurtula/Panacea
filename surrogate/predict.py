...
Example:
    ...
    s.predict({"r0": 2.5, "incubation_period": 5.0, ...})  # returns dict
    s.predict_batch(df_or_2d_array)                         # vectorized
"""
...
class Surrogate:
    ...
    def predict(self, params: Mapping[str, float]) -> dict[str, float]:
        ...
        y = self._forward(x)
        return {c: float(v) for c, v in zip(OUTPUT_COLS, y[0])}
